...
def extractMainLobe(window, M):
    ...
    X = fft(fftbuffer)
    absX = abs(X)
    absX[absX<eps] = eps # Check if there are zero values
    mX = 20*np.log10(absX)

    mX1 = np.zeros(N) # Undo zero phase windowing to visualize the window better
    mX1[:hN] = mX[hN:]
    mX1[N-hN:] = mX[:hN]
    ...
    for i in range(N-hN, N):
        if mX1[i] < mX1[i+1]:
            localminiIndex = i-(N-hN)
            break

    return mX1[(hN - localminiIndex) : (hN + localminiIndex + 1)]
